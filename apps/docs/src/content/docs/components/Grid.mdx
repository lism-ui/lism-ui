...
[`<GridItem>`](/components/griditem/)と組み合わせて使うとより便利です。
...
Gridは、[`grid`オブジェクト内で指定できるプロパティ](/components/props/css-props/#grid) をコンポーネントに直接指定できるようになっています。
...
<Preview>
	...
	<PreviewCode slot='tab' label='HTML'>
		```jsx
		<div class="l--grid -gtc:" style="--gtc:1fr 2fr">
			<div>A</div>
			<div>B</div>
		</div>
		```
...
<Preview>
	...
	<PreviewCode slot='tab' label='HTML'>
		```jsx
		<div
			class="l--grid -gta: -gta@sm -gtc: -gtc@sm -g:20"
