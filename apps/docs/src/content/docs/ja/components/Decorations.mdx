---
title: 'Decorations'
description: ...
---

import { Grid, Stack, Flex, GridItem, FlexItem, Decorator, Box, Callout, Spacer } from '@lism-ui/astro';
import { Reference, MemoBadge, HelpText, DammyText } from "~/components";
import { Preview, PreviewArea, PreviewTitle, PreviewCode } from "~/components/Preview";
import BalloonBox from '~/components/ex/Decorations/balloon';
import PipipiBox from '~/components/ex/Decorations/pipipi';

import '~/components/ex/Decorations/style.scss';

装飾ボックスの作成例を紹介します。

<Callout type='alert'>
このページは準備中です...
</Callout>


{/* 対応クラスは`.c--decoBox--{variant}`。  
(基本的にどういう装飾を行うかの指定が必須で`.c--decoBox`に何かスタイルがあるわけではないので、modifier付きクラス単体で使う） */}


{/* ## Props
variant, subvariant に対しての 各Props をプリセット登録できるようにする予定。プリセット登録せずに装飾内容を指定する場合は `data` に直接指定する。

## Import

```js
import { DecoBox } from '@lism-ui/core';
``` */}

## Usage


### Call

{/* <Box p='30' bd uClass='is--trimHL'>
	<h2>Call</h2>
	<DammyText length='m' />
	<img src='/img/img-1.jpg' alt='' width='960' height='640' />
</Box> */}

<Preview>
	<PreviewTitle>Call</PreviewTitle>
	<PreviewArea resize p='30'>
		<Grid className='b--callBox b--callBox--left' ta='l'>
			<Decorator
				size='1.25em'
				bd='left'
				rotate='45deg'
				transformOrigin='left bottom'
				mr='auto'
			/>
			<Box p='30'>
				<DammyText length='m' />
			</Box>
			<Decorator
				size='1.25em'
				bd='left'
				rotate='-45deg'
				transformOrigin='left top'
				mr='auto'
			/>
		</Grid>
		<Grid className='b--callBox b--callBox--right' as={Grid} ta='r'>
			<Decorator
				size='1.25em'
				bd='right'
				rotate='-45deg'
				transformOrigin='right bottom'
				ml='auto'
			/>
			<Box p='30'>
				<DammyText length='s' />
			</Box>
			<Decorator
				size='1.25em'
				bd='right'
				rotate='45deg'
				transformOrigin='right top'
				ml='auto'
			/>
		</Grid>
		<Spacer h='10' />
		<Flex variant='ltr' className='b--callBox b--callBox--top' jc='center' ta='c'>
			<Decorator
				as={FlexItem}
				fxsh='0'
				size='1.25em'
				bd='top'
				rotate='-45deg'
				transformOrigin='right top'
			/>
			<FlexItem px='20' py='5'>
				<DammyText length='s' />
			</FlexItem>
			<Decorator
				as={FlexItem}
				fxsh='0'
				size='1.25em'
				bd='top'
				rotate='45deg'
				transformOrigin='left top'
			/>
		</Flex>
		<Spacer h='10' />
		<Flex
			variant='ltr'
			className='b--callBox b--callBox--bottom'
			jc='center'
			ai='fe'
			ta='c'
		>
			<Decorator
				as={FlexItem}
				fxsh='0'
				size='1.25em'
				bd='bottom'
				rotate='45deg'
				transformOrigin='right bottom'
			/>
			<FlexItem px='20' py='5' fxg=''>
				<DammyText length='l' />
			</FlexItem>
			<Decorator
				as={FlexItem}
				fxsh='0'
				size='1.25em'
				bd='bottom'
				rotate='-45deg'
				transformOrigin='left bottom'
			/>
		</Flex>
	</PreviewArea>
	<PreviewCode>
		```jsx
		<Grid className='b--callBox b--callBox--left' ta='l'>
			<Decorator
				size='1.25em'
				bd='left'
				rotate='45deg'
				transformOrigin='left bottom'
				mr='auto'
			/>
			<Box p='30'>
				<DammyText length='m' />
			</Box>
			<Decorator
				size='1.25em'
				bd='left'
				rotate='-45deg'
				transformOrigin='left top'
				mr='auto'
			/>
		</Grid>
		<Grid className='b--callBox b--callBox--right' as={Grid} ta='r'>
			<Decorator
				size='1.25em'
				bd='right'
				rotate='-45deg'
				transformOrigin='right bottom'
				ml='auto'
			/>
			<Box p='30'>
				<DammyText length='s' />
			</Box>
			<Decorator
				size='1.25em'
				bd='right'
				rotate='45deg'
				transformOrigin='right top'
				ml='auto'
			/>
		</Grid>
		<Flex variant='ltr' className='b--callBox b--callBox--top' jc='center' ta='c'>
			<Decorator
				as={FlexItem}
				fxsh='0'
				size='1.25em'
				bd='top'
				rotate='-45deg'
				transformOrigin='right top'
			/>
			<FlexItem px='20' py='5'>
				<DammyText length='s' />
			</FlexItem>
			<Decorator
				as={FlexItem}
				fxsh='0'
				size='1.25em'
				bd='top'
				rotate='45deg'
				transformOrigin='left top'
			/>
		</Flex>
		<Flex
			variant='ltr'
			className='b--callBox b--callBox--bottom'
			jc='center'
			ai='fe'
			ta='c'
		>
			<Decorator
				as={FlexItem}
				fxsh='0'
				size='1.25em'
				bd='bottom'
				rotate='45deg'
				transformOrigin='right bottom'
			/>
			<FlexItem px='20' py='5' fxg=''>
				<DammyText length='l' />
			</FlexItem>
			<Decorator
				as={FlexItem}
				fxsh='0'
				size='1.25em'
				bd='bottom'
				rotate='-45deg'
				transformOrigin='left bottom'
			/>
		</Flex>
		```
	</PreviewCode>
</Preview>



### Kakko

<Preview>
	<PreviewTitle>Kakko</PreviewTitle>
	<PreviewArea resize p='30'>
		<Box p='40' pos='r'>
			<DammyText />
			<Decorator size='1.25em' pos='a' t='0' l='0' bd='l,t' />
			<Decorator size='1.25em' pos='a' r='0' b='0' bd='r,b' />
		</Box>
		{/* <Box p='40' pos='r'>
			<DammyText />
			<Decorator size='1.25em' pos='a' t='0' r='0' bd='r,t' />
			<Decorator size='1.25em' pos='a' l='0' b='0' bd='l,b' />
		</Box> */}
		<Box p='40' pos='r'>
			<DammyText />
			<Decorator pos='a' t='0' l='0' w='1em' h='100' bd='l,y' />
			<Decorator pos='a' t='0' r='0' w='1em' h='100' bd='r,y' />
		</Box>
	</PreviewArea>
	<PreviewCode>
	```jsx
	<Box p='40' pos='r'>
		<DammyText />
		<Decorator size='1.25em' pos='a' t='0' l='0' bd='l,t' />
		<Decorator size='1.25em' pos='a' r='0' b='0' bd='r,b' />
	</Box>
	<Box p='40' pos='r'>
		<DammyText />
		<Decorator pos='a' t='0' l='0' w='1em' h='100' bd='l,y' />
		<Decorator pos='a' t='0' r='0' w='1em' h='100' bd='r,y' />
	</Box>
	```
	</PreviewCode>
</Preview>

### Sticky

<Preview>
	<PreviewTitle>Sticky</PreviewTitle>
	<PreviewArea resize p='30'>
		<Grid gd='"content ." / auto 1fr'>
			<GridItem
				ga='content'
				minW='8em'
				maxW='30em'
				bgc='base-2'
				p='40'
				w='fit-content'
				bd='r'
				bdw='.5em'
			>
				<DammyText />
			</GridItem>
			<GridItem
				as={Decorator}
				ga='content'
				w='50%'
				maxW='12em'
				h='1rem'
				ml='auto'
				mt='auto'
				z='-1'
				bgc='hsl(var(--sh-hsl) / 0.25)'
				css={{ filter:'blur(4px)',rotate:'4deg', transformOrigin: 'left bottom', translate: '-4px -4px' }}
			 />
		</Grid>
	</PreviewArea>
	<PreviewCode>
		```jsx
		<Grid gd='"content ." / auto 1fr'>
			<GridItem
				ga='content'
				minW='8em'
				maxW='30em'
				bgc='base-2'
				p='40'
				w='fit-content'
				bd='r'
				bdw='.5em'
			>
				<DammyText />
			</GridItem>
			<GridItem
				as={Decorator}
				ga='content'
				w='50%'
				maxW='12em'
				h='1rem'
				ml='auto'
				mt='auto'
				z='-1'
				bgc='hsl(var(--sh-hsl) / 0.25)'
				css={{ filter:'blur(4px)',rotate:'4deg', transformOrigin: 'left bottom', translate: '-4px -4px' }}
			/>
		</Grid>
		```
	</PreviewCode>
</Preview>

### balloon

<Preview>
	<PreviewTitle>Balloon</PreviewTitle>
	<PreviewArea resize p='30'>
		<BalloonBox variant='left'>
			<DammyText />
		</BalloonBox>
		<BalloonBox variant='right'>
			<DammyText />
		</BalloonBox>
		<BalloonBox variant='top'>
			<DammyText />
		</BalloonBox>
		<BalloonBox variant='bottom'>
			<DammyText />
		</BalloonBox>
		---
		<BalloonBox variant='bottom' bdc='divider' bgc='base-2'>
			<DammyText />
		</BalloonBox>
		<BalloonBox variant='bottom' bdw='2px' boxcolor='red'>
			<DammyText />
		</BalloonBox>
	</PreviewArea>
	<PreviewCode>
	```jsx
	```
	</PreviewCode>
</Preview>



{/* <Preview>
	<PreviewTitle>Pipipi</PreviewTitle>
	<PreviewArea resize p='30'>
		<PipipiBox variant='left'>
			<DammyText />
		</PipipiBox>
		<PipipiBox variant='right'>
			<DammyText />
		</PipipiBox>
	</PreviewArea>
	<PreviewCode>
	```jsx
	```
	</PreviewCode>
</Preview> */}
