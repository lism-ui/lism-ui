---
title: 'Hero'
description: ...
---

import { Spacer, Box, Flex, Grid, Media, Dummy, Text, Container, Layer, Stack, FlexItem, Center, Frame, Button } from '@lism-ui/astro';
import { Preview, PreviewArea, PreviewTitle, PreviewCode, PreviewFrame } from "~/components";
import ShapeDivider from '~/components/ex/ShapeDivider/index.astro';

ヒーローコンテンツの作成例を紹介します。


{/* `<Hero>`内に`<HeroContent>`を使うことで、上・中央・下寄せでコンテンツを配置できます。 */}
{/* <p>親ブロックからbodyブロックを上・中央・下寄せをコントロールできるように。(marginを変える)</p>
<p>初期展開ブロックを選べるようにする。内部gridパターンも。</p> */}


## Usage

{/* `HeroContent` には`type`を指定でき、`top`、`center`、`bottom`のいずれかを指定することができます。  
それぞれのキーワードに合わせて出力されるpropsが変わります。 */}

{/* たとえば、`type='center'`の場合は、`my='auto'`, `hasGutter`, `isContainer='m'`が呼び出されます。 */}

{/* <Stack isFullwide aspect='2/1' bgc='base-2' py='50'>
	<Container size='m' hasGutter isFlow my='auto'>
		<Text fw='bold' fz='xl'>DEMO 01</Text>
		<Dummy length='xl' lang='ja'/>
	</Container>
</Stack> */}

<Preview>
	<PreviewTitle>例 1</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide minH='50vh' bgc='base-2'>
			<Container size='m' hasGutter isFlow my='auto'>
				<Text fw='bold' fz='xl'>DEMO 01</Text>
				<Dummy length='l' lang='ja'/>
			</Container>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx
		<Stack isFullwide minH='50vh' bgc='base-2'>
			<Container size='m' hasGutter isFlow my='auto'>
				<Text fw='bold' fz='xl'>DEMO 01</Text>
				<Dummy length='l' lang='ja'/>
			</Container>
		</Stack>
		```
	</PreviewCode>
</Preview>

<Preview>
	<PreviewTitle>例2: ヘッダー・フッター・レイヤー要素 を追加</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide minH='70vh' g='30' c='white' pos='r'>
			<Frame isLayer>
				<Media isLayer src='/img/img-1.jpg' alt='' width='960' height='640'/>
				<Layer blur='4px' bgc='black:50%' />
			</Frame>
			<Flex jc='sb' p='30' z='0' g='30'>
				<Box>Header</Box>
				<Flex g='30'>
					<div>XXX</div>
					<div>XXX</div>
				</Flex>
			</Flex>
			<Container size='m' hasGutter isFlow my='auto'>
				<Text fw='bold' fz='xl'>DEMO 02</Text>
				<Dummy length='l' />
			</Container>
			<Center p='20' z='0'>Footer</Center>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx
		<Stack isFullwide minH='70vh' g='30' c='white' pos='r'>
			<Frame isLayer>
				<Media isLayer src='/img/img-1.jpg' alt='' width='960' height='640'/>
				<Layer blur='4px' bgc='black:50%' />
			</Frame>
			<Flex jc='sb' p='30' z='0' g='30'>
				<Box>Header</Box>
				<Flex g='30'>
					<div>XXX</div>
					<div>XXX</div>
				</Flex>
			</Flex>
			<Container size='m' hasGutter isFlow my='auto'>
				<Text fw='bold' fz='xl'>DEMO 02</Text>
				<Dummy length='l' />
			</Container>
			<Center p='20' z='0'>Footer</Center>
		</Stack>
		```
	</PreviewCode>
</Preview>

{/* Stack直下なので、position:relativeがなくてもz-indexが効きます。 */}

{/* 
### スクリーンサイズに高さを合わせる

`.-h:full`クラス（`h='full'`） を活用すると、スクリーンサイズに合わせた高さになります。

<Preview>
	<PreviewTitle>例2: top,center,bottom を使用する</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide h='100vh' bgc='base-2' g='30'>
			<Container size='m' hasGutter isFlow my='auto' bd='u:guide'>
				<Text fw='bold' fz='xl'>Full Screen</Text>
				<Dummy length='xl' />
			</Container>
			<Box bd='u:guide' ta='c'>Footer</Box>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx
		<Stack isFullwide h='100vh' bgc='base-2' g='30'>
			<Container size='m' hasGutter isFlow my='auto'>
				<Text fw='bold' fz='xl'>DEMO 03</Text>
				<Dummy length='xl' />
			</Container>
			<Box ta='c'>Footer</Box>
		</Stack>
		```
	</PreviewCode>
</Preview> */}


### `is--layer`, `ShapeDivider` との組み合わせ


{/* <Preview>
	<PreviewTitle>With Layers</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide pos='r' minH='50vh'>
			<Frame isLayer>
				<Media src='/img.jpg' alt='' width='960' height='640'/>
			</Frame>
			<Layer blur='4px' bgc='black:50%' />
			<Container size='m' hasGutter isFlow my='auto' c='white' z='0'>
				<Text fw='bold' fz='xl'>With Layers</Text>
				<Dummy length='l' />
			</Container>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx "\<Frame isLayer"
		<Stack isFullwide pos='r' minH='50vh'>
			<Frame isLayer>
				<Media src='/img.jpg' alt='' width='960' height='640'/>
			</Frame>
			<Media isLayer src='/img.jpg' alt='' width='960' height='640'/>
			<Layer blur='4px' bgc='black:50%' />
			<Container size='m' hasGutter isFlow my='auto' c='white' z='0'>
				<Text fw='bold' fz='xl'>With Layers</Text>
				<Dummy length='l' />
			</Container>
		</Stack>
		```
	</PreviewCode>
</Preview> */}

{/* <Spacer h='10'/> */}

以下の例では、 オプトインコンポーネントの[`<ShapeDivider>`](/ja/components/shapedivider)も使用しています。
<Preview>
	<PreviewTitle>With ShapDivide & Layers</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide hasGutter pos='r' minH='50vh' c='white' g='20'>
			<ShapeDivider viewBox='0 0 10 10' c='base' level='6' scale='-Y' offsetY='-0.5px' z='1'>
				<path d="M10,10V0L0,9.5V10L10,10z"/>
			</ShapeDivider>
			<Frame isLayer>
				<Media isLayer src='/img/img-2.jpg' alt='' width='960' height='640'/>
				<Layer blur='4px' bgc='black:50%' />
			</Frame>
			<Container size='m' isFlow py='40' my='auto' z='0'>
				<Text fw='bold' fz='xl'>ShapDivider & Layers</Text>
				<Dummy length='l' />
			</Container>
			<Box z='0' ta='c'>Footer</Box>
			<ShapeDivider viewBox='0 0 10 10' c='base' level='6' offsetY='0.5px' z='1'>
				<path d="M10,10V0L0,9.5V10L10,10z"/>
			</ShapeDivider>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx "\<Frame isLayer" "\<ShapeDivider"
		<Stack isFullwide hasGutter pos='r' minH='50vh' c='white' g='20'>
			<ShapeDivider viewBox='0 0 10 10' c='base' level='6' scale='-Y' offsetY='-0.5px' z='1'>
				<path d="M10,10V0L0,9.5V10L10,10z"/>
			</ShapeDivider>
			<Frame isLayer>
				<Media isLayer src='/img/img-2.jpg' alt='' width='960' height='640'/>
				<Layer blur='4px' bgc='black:50%' />
			</Frame>
			<Container size='m' isFlow py='40' my='auto' z='0'>
				<Text fw='bold' fz='xl'>ShapDivider & Layers</Text>
				<Dummy length='l' />
			</Container>
			<Box z='0' ta='c'>Footer</Box>
			<ShapeDivider viewBox='0 0 10 10' c='base' level='6' offsetY='0.5px' z='1'>
				<path d="M10,10V0L0,9.5V10L10,10z"/>
			</ShapeDivider>
		</Stack>
		```
	</PreviewCode>
</Preview>

{/* ※ `<Container>`はコンテナ要素なので、z-indexなどを指定しなくてもレイヤーより上に来ますが、ここでは明示的に`z='0'`を指定しています。 */}

<Preview>
	<PreviewTitle>clipPathと組み合わせ、背景に画像があるコンテンツの境界を斜めにつなぐ</PreviewTitle>
	<PreviewArea resize p='30'>
		<Box isFullwide style={{ '--slopeH': 'clamp(2.5rem, 1.25rem + 5cqw, 5rem)' }}>
			<Stack isFullwide hasGutter pos='r' minH='50vh' c='white' g='20'>
				<ShapeDivider viewBox='0 0 10 10' h='var(--slopeH)' c='base' scale='-Y' offsetY='-0.5px' z='1'>
					<path d='M10,10V0L0,9.5V10L10,10z'></path>
				</ShapeDivider>
				<Frame isLayer bgc='black'>
					<Media isLayer src='/img/img-2.jpg' alt='' width='960' height='640' op='0.5' blur='6px'/>
				</Frame>
				<Container size='m' isFlow py='40' pr={['0', '25%', '33%']} my='auto' z='0'>
					<Text fw='bold' fz='xl'>ShapDivider & Layers</Text>
					<Dummy length='l' />
				</Container>
				<Spacer h='var(--slopeH)' />
			</Stack>
			<Stack isFullwide hasGutter pos='r' minH='50vh' c='white' mbs='calc(0px - var(--slopeH))'>
				<Frame
					isLayer bgc='black'
					style={{
						clipPath: 'polygon(0 var(--slopeH), 0 100%, 100% 100%, 100% 0)',
					}}
				>
					<Media isLayer src='/img/img-3.jpg' alt='' width='960' height='640' op='0.5' blur='6px'/>
				</Frame>
				<Spacer h='var(--slopeH)' />
				<Container size='m' isFlow py='40' pl={['0', '25%', '33%']} my='auto' z='0'>
					<Text fw='bold' fz='xl'>ShapDivider & Layers</Text>
					<Dummy length='l' />
				</Container>
				<ShapeDivider viewBox='0 0 10 10' h='var(--slopeH)' c='base' scale='-X' offsetY='0.5px' z='1'>
					<path d='M10,10V0L0,9.5V10L10,10z'></path>
				</ShapeDivider>
			</Stack>
			{/* <Stack isFullwide hasGutter pos='r' minH='50vh' c='white' g='20'>
				<ShapeDivider viewBox='0 0 10 10' h='var(--slopeH)' c='base' scale='-Y' offsetY='-0.5px' z='1' >
					<path d='M10,10V0L0,9.5V10L10,10z'></path>
				</ShapeDivider>
				<Frame isLayer>
					<Media isLayer src='/img/img-2.jpg' alt='' width='960' height='640' />
					<Layer bgc='black' op='0.5'/>
				</Frame>
				<Container size='m' isFlow py='40' pr='25%' my='auto' z='0'>
					<Text fw='bold' fz='xl'>With ShapDivide & Layers</Text>
					<Dummy length='l' />
				</Container>
				<Spacer h='2rem'/>
			</Stack>
			<Stack isFullwide hasGutter pos='r' minH='50vh' c='white'>
				<Frame isLayer z='1' bgc='black' style={{ clipPath: 'polygon(0 var(--slopeH), 100% var(--slopeH), 100% 0)', translate: '0 calc(0.5px - var(--slopeH))' }}>
					<Media src='/img/img-3.jpg' alt='' width='960' height='640' h='calc(100% + var(--slopeH))' op='0.6'/>
				</Frame>
				<Frame isLayer bgc='black'>
					<Media src='/img/img-3.jpg' alt='' width='960' height='640' objectPosition='50% calc(50% - var(--slopeH))' h='calc(100% + var(--slopeH))' op='0.6' />
				</Frame>
				<Container size='m' isFlow py='40' pl='25%' my='auto' z='0'>
					<Text fw='bold' fz='xl'>With ShapDivide & Layers</Text>
					<Dummy length='l' />
				</Container>
				<ShapeDivider viewBox='0 0 10 10' h='var(--slopeH)' c='base' scale='-X' offsetY='0.5px' z='1'>
					<path d='M10,10V0L0,9.5V10L10,10z'></path>
				</ShapeDivider>
			</Stack> */}
		</Box>
	</PreviewArea>
	<PreviewCode>
		```jsx "--slopeH"
		<Box isFullwide style={{ '--slopeH': 'clamp(2.5rem, 1.25rem + 5vw, 5rem)' }}>
			<Stack isFullwide hasGutter pos='r' minH='50vh' c='white' g='20'>
				<ShapeDivider viewBox='0 0 10 10' h='var(--slopeH)' c='base' scale='-Y' offsetY='-0.5px' z='1'>
					<path d='M10,10V0L0,9.5V10L10,10z'></path>
				</ShapeDivider>
				<Frame isLayer bgc='black'>
					<Media isLayer src='/img/img-2.jpg' alt='' width='960' height='640' op='0.5' blur='6px'/>
				</Frame>
				<Container size='m' isFlow py='40' pr={['0', '25%', '33%']} my='auto' z='0'>
					<Text fw='bold' fz='xl'>ShapDivider & Layers</Text>
					<Dummy length='l' />
				</Container>
				<Spacer h='var(--slopeH)' />
			</Stack>
			<Stack isFullwide hasGutter pos='r' minH='50vh' c='white' mbs='calc(0px - var(--slopeH))'>
				<Frame
					isLayer bgc='black'
					style={{
						clipPath: 'polygon(0 var(--slopeH), 0 100%, 100% 100%, 100% 0)',
					}}
				>
					<Media isLayer src='/img/img-3.jpg' alt='' width='960' height='640' op='0.5' blur='6px'/>
				</Frame>
				<Spacer h='var(--slopeH)' />
				<Container size='m' isFlow py='40' pl={['0', '25%', '33%']} my='auto' z='0'>
					<Text fw='bold' fz='xl'>ShapDivider & Layers</Text>
					<Dummy length='l' />
				</Container>
				<ShapeDivider viewBox='0 0 10 10' h='var(--slopeH)' c='base' scale='-X' offsetY='0.5px' z='1'>
					<path d='M10,10V0L0,9.5V10L10,10z'></path>
				</ShapeDivider>
			</Stack>
		</Box>
		```
	</PreviewCode>
</Preview>



### 子要素の`minH`に`%`を使う方法

{/* - l-stack(flex)の場合
	- Heroの高さを h でセット→ HeroContent は h も minH,maxH も fxb も % が使用できる.
	- Heroの高さを minH でセット → HeroContent は h も minH,maxH もfxb も % は無効になる。
	- Heroのアスペクト比をセット→ HeroContent は h も minH,maxH も fxb も % が使用できる.
		- minH, fxb を使うと、小さいウインドウではコンテンツに合わせてHeroの高さが可変する。
		- h,maxH を使うと、小さいウインドウではHeroContentの高さが外に溢れていく（ovy='auto'で内部スクロールできる）
- l-gridの場合
	- 何で高さを確保しても、HeroContent の高さは%が使える。
	- が、そのさらに子要素のhに%を使うには、結局 HeroContent をGrid にする必要がある。Layerを使ってもこれは一緒。
	（ HeroContent 自体を色々装飾するとpaddingの位置が問題になってくるので、HeroContentの子要素の高さに%をつかえるのが大事。）

→ 子要素に%を使うための複雑性はあまりかわらないので、その他の運用がシンプルになる stack の構成で決定。 */}

{/* -  */}

{/* Hero 自体の高さを`height`で固定している場合は、普通に子要素であるHeroContentや、そのさらに子要素の`height`や`min-height`にも`%`を使うことができます。

<Preview>
	<PreviewTitle>親の高さが`h`で指定されるケース</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide h='400px' bgc='base-2'>
			<Box h='50%' my='auto' px='40'>
				<Center h='100%' bgc='base' bxsh='1'>
					<Text fw='bold'>Height 50%</Text>
				</Center>
			</Box>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx "h='400px'" "h='50%'"
		<Stack isFullwide h='400px' bgc='base-2'>
			<Box h='50%' my='auto' px='40'>
				<Center h='100%' bgc='base' bxsh='1'>
					<Text fw='bold'>Height 50%</Text>
				</Center>
			</Box>
		</Stack>
		```
	</PreviewCode>
</Preview> */}


最低限の高さを確保しつつ、ウインドウが狭い時にコンテンツに合わせて高さを伸ばしたいケースについて。

Heroコンテンツの高さを `min-height` や `aspect-ratio` で指定する場合、子要素の `height` や `min-height` に%を指定しても無効になってしまいます。

{/* (ウインドウ幅が広い時は特定の高さに制限したいが、ウインドウ幅が狭くなって内部のコンテンツ量がはみ出す時は高さが伸びてくれるようなレイアウトを作りたいケース。)

基本的に、`min-height`を指定した要素の子要素では、`height`や`min-height`に`%`を使っても無効となります。 */}


<Preview>
	<PreviewTitle>NG例</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide minH='400px' bgc='base-2'>
			<Box minH='50%' my='auto' bgc='base' p='20' px='40' ac='c'>
				<Text fw='bold'>minH:50%が効かない</Text>
				<Dummy length='s' />
			</Box>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx "minH='400px'" "minH='50%'"
		<Stack isFullwide minH='400px' bgc='base-2'>
			<Box minH='50%' my='auto' bgc='base' p='20' px='40' ac='c'>
				<Text fw='bold'>minH:50%が効かない</Text>
				<Dummy length='s' />
			</Box>
		</Stack>
		```
	</PreviewCode>
</Preview>


このため、少し工夫が必要です。

<Preview>
	<PreviewTitle>改善例</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide minH='400px' bgc='base-2'>
			<FlexItem d='grid' fxg='1' ai='c' px='40'>
				<Center minH='50%' bgc='base' p='30' bxsh='1'>
					<Text fw='bold'>OK</Text>
					<Dummy length='s' />
				</Center>
			</FlexItem>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx "d='grid'" "fxg='1' ai='c'" "minH='50%'"
		<Stack isFullwide minH='400px' bgc='base-2'>
			<FlexItem d='grid' fxg='1' ai='c' px='40'>
				<Center minH='50%' bgc='base' p='30' bxsh='1'>
					<Text fw='bold'>OK</Text>
					<Dummy length='s' />
				</Center>
			</FlexItem>
		</Stack>
		```
	</PreviewCode>
</Preview>


1. 親要素が`Stack` で構成されているので、子要素であるコンテンツの`flex-grow`を`1`にして高さを親に合わせて伸ばします。
2. 子要素を`grid`にすることで、さらにその子要素で`height`や`min-height`に`%`を使えるようにします。  
	（この時、その子要素を中央寄せ等にしないと`stretch`が適用されて高さが伸びることに注意してください。）
	{/* - grid要素に`align-items:center`などを指定する。
	- grid要素の子要素に`margin-block:auto` か `align-self:center`を指定する。 */}


{/* `type="stretch"`を指定すると上記の工程を手軽に呼び出せます。（`as={Grid} fxg='1' ai='center'` をセットする） */}


<Preview>
	<PreviewTitle>活用例</PreviewTitle>
	<PreviewArea resize>
		<Stack isFullwide pos='r' minH='50vh'>
			<Frame isLayer>
				<Media src='/img/img-3.jpg' alt='' width='960' height='640'/>
			</Frame>
			<Layer bgc='black' op='0.2' />
			<FlexItem layout={Grid} pos='r' fxg='1' ai='c' hasGutter>
				<Center minH='50%' p='30' c='white' bgc='black:70%' bdrs='2' g='20'>
					<Text fw='bold' fz='xl'>min-height 50%</Text>
					<Dummy length='l' />
				</Center>
			</FlexItem>
		</Stack>
	</PreviewArea>
	<PreviewCode>
		```jsx
		<Stack isFullwide pos='r' minH='50vh'>
			<Frame isLayer>
				<Media src='/img/img-3.jpg' alt='' width='960' height='640'/>
			</Frame>
			<Layer bgc='black' op='0.2' />
			<FlexItem layout={Grid} fxg='1' ai='c' hasGutter>
				<Center minH='50%' p='30' c='white' bgc='black:70%' bdrs='2' g='20'>
					<Text fw='bold' fz='xl'>min-height 50%</Text>
					<Dummy length='l' />
				</Center>
			</FlexItem>
		</Stack>
		```
	</PreviewCode>
</Preview>



## 別ページで確認する
<PreviewFrame url='/demo/hero' />
