...
| `size` | プリセットアイコンを呼び出す場合に、`width`, `height`として出力されます。 `as`で外部コンポーネントを指定している場合Lism Props ではなくその外部コンポーネントに渡されます。それ以外の場合は、Lism Propsとして処理されます。|
...
<Preview>
	...
	<PreviewCode slot='tab' label="HTML">
		```html
		<span aria-hidden="true" class="e--icon e--icon--emoji -fz:" style="--fz:2em;">✍️</span>
		<span aria-hidden="true" class="e--icon e--icon--emoji -fz:" style="--fz:2em;">💡</span>
		```
	</PreviewCode>
</Preview>
