...
description: ...
---
import { LinkBox, OverlayLink, Stack, Callout, Text, Spacer } from '@lism-ui/astro';
...
2. `<LinkBox>`が`<div>`で、内部に`a--overlayLink`を配置する。
3. `<LinkBox>`が`<div>`で、内部に`a--hiddenLink`を配置する。
...
### 内部に`a--overlayLink`を配置する

LinkBoxに`href`を指定せずに使うと、`<a>`ではなく`<div>`で出力され、  
内部のアンカー要素に`.a--overlayLink`を併用することで`.is--linkBox`全体をリンクにすることができます。
...
`a--overlayLink`にフォーカスした時のアウトラインをLinkBox全体に広げたい場合は、次のようなCSSを追記してください。

```css
@supports selector(:has(*)) {
	.is--linkBox:has(.a--overlayLink:focus-visible) {
		outline: auto 1px;
...
<Preview>
	...
	<PreviewCode slot='tab' label='HTML'>
		```jsx {2,8} "a--hiddenLink"
		<div class="is--linkBox -hov:fade -p:box:l -bgc:base -bd: -bdc:divider -bdrs:l">
			<a class="a--hiddenLink is--skipFlow" href="#boxlink03" aria-label='リンクだよ'></a>
			<p class="-fz:xl -fw:bold">Heading Text</p>
			<p class="-c:content-2 -lh:s -mbs:20">
				Lorem ipsum dolor sit, amet consectetur adipisicing elit, sed do eiusmod tempor. Non facere laudantium ex eos doloribus aut dolore nisi provident.
			</p>
