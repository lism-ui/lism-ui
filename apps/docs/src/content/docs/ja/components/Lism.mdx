...
description: ...
---
import { Lism, Text, Dummy, Link, Media, Frame } from "@lism-ui/astro";
...
この時、[Prop Class](/ja/css/prop-class/) に対応する値が指定されれば、上記のようにクラスが出力されるようになっています。
...
ブレイクポイントでの切り替えに対応しているような主要なプロパティは、2. の`-{prop}:` & `--{prop}` で出力されます。

{/* <p class='-fz:s -op:high -mbs:5'>そのほか、(変数管理できた方が利便性が高いので)主要なプロパティも変数化されるようになっています。</p> */}


<Preview>
<PreviewTitle>ブレイクポイントに対応したプロパティの出力例</PreviewTitle>
<PreviewCode>
```jsx
...
<Preview>
	<PreviewTitle>filter系のプロパティ, objectPositionの指定</PreviewTitle>
	<PreviewArea>
