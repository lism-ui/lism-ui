...
import { Preview, PreviewArea, PreviewTitle, PreviewCode, ImportPackage, PropBadge, Callout, IconBadge } from "~/components";
...
`d--tabs__tab`では`--_isActive`, `--_notActive`が使えるようになっています。  
<IconBadge type='help'/> （[`-hov:set`](/ja/css/hover#hov-set)の`--_isHov`,`--_notHov`の解説を参考にしてください。）
