...
import { Callout, Box, Text, Lism, Stack, Flex, Container, Frame, Button, Center, Spacer } from '@lism-ui/core';
import { Accordion,AccordionHeader, AccordionBody } from '@lism-ui/astro';
...
|---|---|
| `.is--flow` | `--flowGap--m` |
| `.is--flow.-flowGap:s` | `--flowGap--s` |
| `.is--flow.-flowGap:` | `--flowGap`で任意の値をセットできます。 |
{/* | `.is--flow.-flowGap:l` | `--flowGap--l` (`--s--50`) | */}
...
<Preview>
	...
	<div class="is--flow -p:10">
		<p>Content...</p>
		<p>Content...</p>
		<div class="is--flow -flowGap:s -p:10">
			<p>Content...</p>
			<p>Content...</p>
			<p>Content...</p>
		</div>
...
任意の余白量をセットしたい場合は、`-flowGap:`クラスと`--flowGap`変数を組み合わせます。
...
<Preview>
	...
	<PreviewCode slot='tab' label='HTML'>
	```html
	<div class="is--flow -flowGap:" style="--flowGap:10px">
		<p>Contents...</p>
		<p>Contents...</p>
		<p>Contents...</p>
	</div>
...
`-flowGap:`の直下でさらに`-flowGap:`を配置すると、子要素側が自身の `--flowGap` に干渉してしまいます。
...
<Callout type='info'>
	`.is--flow`, `.is--flow.-flowGap:s` をネストするのは問題ありません.
</Callout>
...
子要素の間に仕切り線を表示します。デフォルトでは`border-bottom`で区切り線が描画されます。
...
`--isRow`はブレイクポイントの指定に対応しているため、`flex-direction`の切り替えに合わせて区切り線の方向を変えることができます。

<Preview>
	<PreviewTitle>`--isRow`で方向を管理する</PreviewTitle>
	<PreviewArea resize p='20' nogrid>
		<Flex hasDivider fxd={['column','row']} isRow={[0,1]} bdw='2px' bdc='blue'>
			<Box px='20' py='5'>Content</Box>
			<Box px='20' py='5'>Content</Box>
			<Box px='20' py='5'>Content</Box>
		</Flex>
	...
	```html
	<div
		class="l--flex has--divider -isRow@sm -fxd:c -fxd@sm"
		style="--isRow:0;--sm-isRow:1;--bdw:2px;--bdc:var(--blue);--sm-fxd:row">
		<div class="-px:em10">Content</div>
		<div class="-px:em10">Content</div>
		<div class="-px:em10">Content</div>
	</div>
