...
sidebar:
  order: 1
---
import { Box, Callout } from "@lism-ui/astro";
import { Preview, PreviewArea, PreviewTitle, PreviewCode, Reference, HelpText } from "~/components";
...
<table className="b--table" style={{'--bdwX': '1px'}}>
	<thead>
		<tr>
...
|分類|形式|例|
|---|---|---|
|Layout<HelpText tag='small'>レイアウトの構成単位となるようなもの</HelpText>|`.l--{layoutName}`|`.l--grid`, `.l--columns`|
|Atom<HelpText tag='small'>コンテンツの構成要素として最小単位となるようなもの</HelpText>|`.a--{atomName}`|`.a--icon`|
|Block<HelpText tag='small'>シンプルな役割を持つコンテンツ要素</HelpText>|`.b--{atomName}`|`.b--avatar`,`.b--button`|
|Component<HelpText tag='small'>その他のコンポーネント</HelpText>|`.c--{componentName}`|`.c--card`, `.c--tab`|
...
<div class="-p:20 sm_p: md_p: -bd:" style="--sm-p:var(--s--30);--md-p:var(--s--50);">
	BOX
</div>
```
	</PreviewCode>
