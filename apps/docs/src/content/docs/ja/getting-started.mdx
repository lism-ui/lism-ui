---
title: 'Getting Started'
description: '...'
---

import { Badge, Callout, Tabs, TabItem } from '@lism-ui/astro';
import { SmileyXEyes } from "@phosphor-icons/react";
import { MemoBadge } from "~/components";
import { Preview, PreviewArea, PreviewTitle, PreviewCode } from "~/components/Preview";


## Installation

Lismは2通りの使い方ができます。

### 1. CSS設計を導入したいだけの場合

HTMLで組むシンプルなサイトであれば、CDN経由でCSSを読み込むだけで利用できます。

```html
<link href="https://cdn.jsdelivr.net/npm/@lism-ui/core/dist/css/all.css" rel="stylesheet" />
```

もちろん、CSSファイルをダウンロードして読み込んでもOKです。

### 2. パッケージを使ってLism UI をフル活用する場合

ReactやAstroで専用のコンポーネントを使用するためには、コアパッケージをインストールする必要があります。

<Tabs>
	<TabItem label='npm'>
		```bash frame="none"
		npm i @lism-ui/core
		```
	</TabItem>
	<TabItem label='pnpm'>
		```bash frame="none"
		pnpm add @lism-ui/core
		```
	</TabItem>
		<TabItem label='yarn'>
		```bash frame="none"
		yarn add @lism-ui/core
		```
	</TabItem>
</Tabs>


### CSSの読み込み

例えば、Next.jsであれば、`_app.js`や`layout.js`でglobalスタイルとして読み込んでください。例えば、Next.jsであれば、`_app.js`や`layout.js`でglobalスタイルとして読み込んでください。例えば、Next.jsであれば、`_app.js`や`layout.js`でglobalスタイルとして読み込んでください。

```js
import '@lism-ui/core/css/all.css';
```


### Purgeする

ビルド環境の調整ができる場合は、[PurgeCSS](https://purgecss.com/)を利用して、必要なCSSだけを抽出することができます。<MemoBadge>未検証</MemoBadge>



## Using Components

```jsx
import { Box, Text, Box, Text, Box, Text, Box, Text, Box, Text, Box, Text, Box, Text, ... } from '@lism-ui/core';

// ...
<Box p={20} bgc='base-2'>
	<Text fz="l">Lorem ipsum text...</Text>
</Box>
// ...
```

{/* <Reference></Reference> */}

## 特定の環境に合わせた コンポーネントの読み込み

### Next.js (App Router)

{/* Next.js の **app ルーティング**を使用している場合、`<Accordion>`や`<Tab>`を使うには`"use client"`の記述が必要になりますが、 */}


`@lism-ui/next`パッケージを追加すると、Lism コンポーネントに関する`"use client"`の記述を省略することができるようになります。

<small>（`"use client"`が必要なものにはすでに記述された状態でimportできるようになります。）</small>


```bash frame="none" filename="Add package"
npm i @lism-ui/next
```

<Callout type='warning'>
`@lism-ui/next`から`@lism-ui/core`のコードを ンポートできるようにするため、`next.config.js`の`transpilePackages`に`"@lism-ui/next"`を追加してください。
</Callout>


```js filename="next.config.js"
const nextConfig = {
	transpilePackages: ["@lism-ui/next"],
	// ...
};

module.exports = nextConfig;
```


これらの準備により、`<Tabs>`などの`"use client"`が必要なコンポーネントも簡単に利用できるようになります。  
（Boxなどの普通のコンポーネントも、`@lism-ui/next`から同時にimportできます。）

```js
import { Box, Tabs, TabItem } from '@lism-ui/next';

<Tabs>
	<TabItem label='tab1'>
		<Box>...</Box>
	</TabItem>
	<TabItem label='tab2'>
		<Box>...</Box>
	</TabItem>
</Tabs>
```



### Astro

`@lism-ui/astro`パッケージを追加すると、`.jsx`ファイルではなく、`.astro`ファイルでもコンポーネントを利用できるようになります。<MemoBadge>一部準備中</MemoBadge>

以下のパッケージを追加でインストールしてください。

```bash frame="none"
npm i @lism-ui/astro
```

```js
import { Stack, Flex } from '@lism-ui/astro';
```


<Callout type='warning'>
`@lism-ui/astro`から`@lism-ui/core`のコードをインポートできるようにするため、`astro.config.js`の`vite.ssr.noExternal`に`"@lism-ui/core"`を追加してください。
</Callout>

```js filename="astro.config.js"
import { defineConfig } from "astro/config";

export default defineConfig({
  // ...other settings 
  vite: {
    ssr: {
      noExternal: ["@lism-ui/core"],
    },
  },
});
```

この記述がないと、"**Cannot use import statement outside a module**" というエラーが発生します。  
<small>（パッケージ配布方法の改善で対応できるかどうか調整中です...）</small>


また、一部のコンポーネントは、ReactとAstroで利用方法が変わるので注意してください。（各コンポーネントの解説ページで個別に説明していく予定です。）

{/* 
## lism.config のセッティング

Lism Propsのカスタマイズなどを行うためのファイルです。  
トークン値のカスタマイズや、どの値をユーティリティクラス化するかを設定できます。<MemoBadge>未実装</MemoBadge>

```js filename="lism.config.js"
...
``` */}
