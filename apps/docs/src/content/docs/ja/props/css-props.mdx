...
sidebar:
  order: 3
---
import { Box, Grid, Note } from "@lism-ui/astro";
...
以下のものは `gridItem`, `flexItem` オブジェクト内で指定することができます。


|Prop|CSS|To Class|BP|
|---|---|---|---|
|`aslf`|`align-self`|`c`, `s`, `e`, `fs`, `fe`|-|
|`jslf`|`justify-self`|`c`, `s`, `e`, `fs`, `fe`|-|
|`pslf`|`place-self`|-|-|
|`ord`|`order`|`-1`, `0`, `1`|-|
...
```jsx
<Lism d='grid' ai='center' grid={{gtc:'1fr 1fr', gap:40}}>...</Lism>
```
...
<Grid ai='center'gtc='1fr 1fr' gap={40}>...</Grid>
```
{/* <Grid gtc='1fr 1fr' gap={40} ai='center'>
<Box>aaa<br/>bbb</Box>
<Box>aaa</Box>
<Box>aaa</Box>
</Grid> */}
