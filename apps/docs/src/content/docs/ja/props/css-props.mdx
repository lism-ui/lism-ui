...
sidebar:
  order: 3
---
import { Lism, Box, Grid } from "@lism-ui/astro";
...
|---|---|---|---|---|
|`c`|`color`|`COLOR`|`text`,`text-2`,`base`,`keycolor`,`i`,`mix`|-|
|`bgc`|`background-color`|`COLOR`|`base`,`base-2`,`text`,`keycolor`,`i`,`mix`|-|
|`bdc`|`border-color`|`COLOR`|`divider`,`keycolor`,`i`,`mix`|-|
...
### コンポーネントでの `mix` クラスの呼び出し方

`.-c:mix` や `.-bgc:mix` クラスは、3つの変数を受け取とるようになっています。

`c='mix'`のようにしてCSS変数をstyleで定義してもよいですが、`'color1:(color2:)NUM%'` の書式で指定することで、自動で変数に分割されるようになっています。
...
|px|padding-inline|`SPACE`|`SPACE`|○|
|py|padding-block|`SPACE`|`SPACE`|○|
|pis|padding-inline-start|`SPACE`|`SPACE`|○|
...
|`bdc`|`border-color`|`COLOR.PALETTE`, `COLOR.BD`|`divider`,`t`,`i`,`cc`,`keycolor`|-|
|`bdw`|`border-width`|-|-|-|
|`bds`|`border-style`|-|-|-|
{/* |`bdl`|`border-left`|-|-|-|
|`bdr`|`border-right`|-|-|-|
